...
namespace cobra {
    ...
    struct ExponentTuple
    {
        ...
        void ToExponents(
            uint8_t *out, uint8_t num_vars
        ) const { // NOLINT(readability-non-const-parameter) - out is written to
            uint32_t p = packed;
        ...
        ExponentTuple WithExponent(uint8_t var_index, uint8_t new_val, uint8_t num_vars) const {
            ...
            const uint32_t kPos = kPow3[num_vars - 1 - var_index];
            const auto kOldVal  = static_cast< uint8_t >((packed / kPos) % 3);
            return { packed - (kOldVal * kPos) + (new_val * kPos) };
        }
