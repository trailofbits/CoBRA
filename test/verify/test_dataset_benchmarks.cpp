...
TEST(GAMBADataset, QSynthEA) {
    ...
    EXPECT_EQ(stats.parsed, 500);
    EXPECT_EQ(stats.simplified, 463);
    EXPECT_EQ(stats.unsupported, 37);
    EXPECT_EQ(stats.failed_simplify, 0);
    ...
    EXPECT_EQ(stats.by_category[ReasonCategory::kGuardFailed], 8);
    EXPECT_EQ(stats.by_category[ReasonCategory::kSearchExhausted], 29);
