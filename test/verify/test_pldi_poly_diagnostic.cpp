...
TEST(PLDIPolyDiagnostic, AnalyzeUnsolved) {
    ...
    std::cerr << "\n--- By route ---\n";
    for (auto &[k, v] : by_route) {
        std::cerr << "  " << k << ": " << v << "\n";
    }

    std::cerr << "\n--- By variable count ---\n";
    for (auto &[k, v] : by_vars) {
        std::cerr << "  " << k << " vars: " << v << "\n";
    }

    std::cerr << "\n--- By GT depth ---\n";
    for (auto &[k, v] : by_depth) {
        std::cerr << "  depth " << k << ": " << v << "\n";
    }

    std::cerr << "\n--- By GT binary op count ---\n";
    for (auto &[k, v] : by_binary_ops) {
        std::cerr << "  " << k << " binops: " << v << "\n";
    }
