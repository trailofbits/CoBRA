...
TEST(SimplifierTest, kPolynomialCancelledToLinear) {
...
TEST(SimplifierTest, kPolynomialTargetFailsFullWidth) {
...
TEST(SimplifierTest, kPolynomialTargetRecoveredViaSplit) {
...
TEST(SimplifierTest, kPolynomialXSquaredRecovered) {
...
TEST(SimplifierTest, kPolynomialMixedXYPlusLinear) {
    // f(x,y) = x*y + 2*x + 3*y + 1
...
TEST(SimplifierTest, ProductIdentityPlusLinearResidual) {
    ...
    e      = Expr::Add(
        std::move(e),
        Expr::Negate(Expr::Mul(Expr::Constant(10), Expr::BitwiseAnd(x(), Expr::BitwiseNot(y())))
             )
    );
    e = Expr::Add(
...
TEST(SimplifierTest, ProductIdentityPlusLinearResidual3Var) {
    ...
    e      = Expr::Add(
        std::move(e),
        Expr::Negate(Expr::Mul(
            Expr::Constant(6),
            Expr::BitwiseNot(Expr::BitwiseOr(Expr::BitwiseNot(x()), Expr::BitwiseAnd(y(), z())))
        ))
    );
    // + 5 * ~(~x | (y | z))
    e = Expr::Add(
    ...
    e = Expr::Add(
        ...
        Expr::Mul(
            Expr::Constant(6),
            Expr::BitwiseNot(Expr::BitwiseOr(
