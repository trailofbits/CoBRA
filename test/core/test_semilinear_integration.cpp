...
TEST(kSemilinearIntegration, CarryErasure) {
    // (x & 0xFF) + 1
    auto input =
...
TEST(kSemilinearIntegration, SpuriousVarOnBoolean) {
    // (x & 0xFF) + (y & 0xFF00)
    auto input = Expr::Add(
...
TEST(kSemilinearIntegration, kLinearInDisguise) {
    // (x & 0xFF) + (x | 0xFF)
    auto input = Expr::Add(
        Expr::BitwiseAnd(Expr::Variable(0), Expr::Constant(0xFF)),
...
TEST(kSemilinearIntegration, NonOverlappingMasks) {
    // (x & 0x00FF) + (y & 0xFF00)
    auto input = Expr::Add(
...
TEST(kSemilinearIntegration, CoefficientPreventsOR) {
    // 3 * (x & 0x0F) + (y & 0xF0)
    auto input = Expr::Add(
...
TEST(kSemilinearIntegration, ShrAtomRoundTrip) {
...
TEST(kSemilinearIntegration, ShrWithCoeffAndAdd) {
    auto shr_atom =
...
TEST(kSemilinearIntegration, ShrIdentityFold) {
