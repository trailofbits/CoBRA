...
namespace cobra {
    ...
    namespace {
        ...
        Result< std::vector< Token > > Tokenize(const std::string &expr) {
            ...
            while (i < expr.size()) {
                    ...
                    && (expr[i + 1] == 'x' || expr[i + 1] == 'X'))
                {
                    ...
                    tokens.push_back({ .type        = TokenType::kNumber,
                ...
                if (std::isdigit(expr[i]) != 0) {
                    const size_t start = i;
                    while (i < expr.size() && (std::isdigit(expr[i]) != 0)) {
                        ++i;
                    }
                    tokens.push_back({ .type        = TokenType::kNumber,
                ...
                if ((std::isalpha(expr[i]) != 0) || expr[i] == '_') {
                    ...
                    tokens.push_back({ .type        = TokenType::kVariable,
                                       .value       = expr.substr(start, i - start),
                                       .precedence  = 0,
                                       .right_assoc = false,
                                       .is_unary    = false });
                ...
                if (expr[i] == '(') {
                    tokens.push_back({ .type        = TokenType::kLParen,
                                       .value       = "(",
                                       .precedence  = 0,
                                       .right_assoc = false,
                                       .is_unary    = false });
                ...
                if (expr[i] == ')') {
                    tokens.push_back({ .type        = TokenType::kRParen,
                                       .value       = ")",
                                       .precedence  = 0,
                                       .right_assoc = false,
                                       .is_unary    = false });
                ...
                if (expr[i] == '<' && i + 1 < expr.size() && expr[i + 1] == '<') {
                    tokens.push_back({ .type        = TokenType::kOp,
                                       .value       = "<<",
                                       .precedence  = 4,
                                       .right_assoc = false,
                                       .is_unary    = false });
                    ...
                    if (tok.is_unary) {
                        ...
                        if (tok.value == "<<" || tok.value == ">>") {
                            ...
                            if (tok.value == "<<") {
                                ...
                                stack.push(Expr::Mul(std::move(lhs), Expr::Constant(multiplier))
                                );
                            } else {
        ...
        Result< void > ValidateShifts(const std::vector< Token > &postfix, uint32_t bitwidth) {
            for (size_t i = 0; i < postfix.size(); ++i) {
                if (postfix[i].type != TokenType::kOp) {
                    continue;
                }
    ...
    Result< ParseResult > ParseAndEvaluate(const std::string &expr, uint32_t bitwidth) {
        ...
        if (!shifts_ok) {
            return std::unexpected(std::move(shifts_ok.error()));
        }
        ...
        for (const auto &tok : *postfix) {
            if (tok.type == TokenType::kNumber) {
                ...
                compiled.push_back({ .op = CompiledOp::kPushConst, .operand = num_val & mask });
                ++depth;
            } else if (tok.type == TokenType::kVariable) {
            ...
            for (const auto &ct : compiled) {
                switch (ct.op) {
                    ...
                    case CompiledOp::kPushVar:
                        stack.push_back(inputs[ct.operand] & mask);
                        break;
                    case CompiledOp::kNot:
                        stack.back() = (~stack.back()) & mask;
                        break;
