...
namespace cobra {
    ...
    namespace {
        ...
        std::optional< std::unique_ptr< Expr > > Match3varBoolean(uint8_t key) {
            ...
            switch (key) {
                ...
                case 0x6B: // y ^ ~((x ^ z) & (y | z))
                    return Expr::BitwiseXor(
                        y(),
                        Expr::BitwiseNot(Expr::BitwiseAnd(
                            Expr::BitwiseXor(x(), z()), Expr::BitwiseOr(y(), z())
                        ))
                    );
                case 0x6C: // y ^ (x & z)
        ...
        std::optional< std::unique_ptr< Expr > > MatchScaledBoolean(
            ...
            if (k != 1) {
                result = Expr::Mul(Expr::Constant(k), std::move(result));
            }
            if (c != 0) {
                result = Expr::Add(Expr::Constant(c), std::move(result));
            }
            return result;
        }
    ...
    MatchPattern(const std::vector< uint64_t > &sig, uint32_t num_vars, uint32_t bitwidth) {
        ...
        if (num_vars == 5 && IsBooleanSig(sig)) {
            return Match5varBoolean(PackBoolSig(sig));
        }

        // Scaled boolean: c + k * boolean_pattern
        if (!IsBooleanSig(sig)) {
            return MatchScaledBoolean(sig, num_vars, bitwidth);
        }
