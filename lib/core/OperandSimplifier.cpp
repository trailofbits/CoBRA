...
namespace cobra {
    namespace {
        ...
        std::unique_ptr< Expr >
        MakeMulSimplified(std::unique_ptr< Expr > lhs, std::unique_ptr< Expr > rhs) {
            if (IsConstant(*lhs, 0) || IsConstant(*rhs, 0)) {
        ...
        std::unique_ptr< Expr >
        MakeAddSimplified(std::unique_ptr< Expr > lhs, std::unique_ptr< Expr > rhs) {
            if (IsConstant(*lhs, 0)) {
                return rhs;
            }
            if (IsConstant(*rhs, 0)) {
        ...
        std::unique_ptr< Expr > MakeAndSimplified(
            ...
            if (IsConstant(*lhs, mask)) {
                return rhs;
            }
            if (IsConstant(*rhs, mask)) {
                return lhs;
            }
        ...
        std::unique_ptr< Expr >
        MakeXorSimplified(std::unique_ptr< Expr > lhs, std::unique_ptr< Expr > rhs) {
            if (IsConstant(*lhs, 0)) {
                return rhs;
            }
            if (IsConstant(*rhs, 0)) {
        ...
        std::unique_ptr< Expr > RebuildNode(
            Expr::Kind kind, std::vector< std::unique_ptr< Expr > > children,
        ...
        std::optional< std::unique_ptr< Expr > > TrySimplifyOperand(
            ...
            if (!IsBetter(result->cost, operand_cost)) {
                return std::nullopt;
            }
