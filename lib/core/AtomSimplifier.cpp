...
namespace cobra {
    ...
    namespace {
        ...
        std::unique_ptr< Expr > TryFoldBinary(
            ...
            const bool kLc          = IsConst(*lhs);
            const bool kRc          = IsConst(*rhs);
            const uint64_t kAllOnes = Bitmask(bitwidth);

            if (kind == Expr::Kind::kAnd) {
                if (kRc && rhs->constant_val == 0) { return Expr::Constant(0); }
                if (kLc && lhs->constant_val == 0) { return Expr::Constant(0); }
                if (kRc && rhs->constant_val == kAllOnes) { return lhs; }
                if (kLc && lhs->constant_val == kAllOnes) { return rhs; }
            }

            if (kind == Expr::Kind::kOr) {
                if (kRc && rhs->constant_val == 0) { return lhs; }
                if (kLc && lhs->constant_val == 0) { return rhs; }
                if (kRc && rhs->constant_val == kAllOnes) { return Expr::Constant(kAllOnes); }
                if (kLc && lhs->constant_val == kAllOnes) { return Expr::Constant(kAllOnes); }
            }
        ...
        bool ExprsEqual(const Expr &a, const Expr &b) {
            ...
            if (a.kind == Expr::Kind::kConstant) { return a.constant_val == b.constant_val; }
            if (a.kind == Expr::Kind::kVariable) { return a.var_index == b.var_index; }
            if (a.children.size() != b.children.size()) { return false; }
            for (size_t i = 0; i < a.children.size(); ++i) {
                if (!ExprsEqual(*a.children[i], *b.children[i])) { return false; }
            }
            return true;
        }
    ...
    std::unique_ptr< Expr > SimplifyAtom(std::unique_ptr< Expr > atom, uint32_t bitwidth) {
        ...
        for (auto &child : atom->children) { child = SimplifyAtom(std::move(child), bitwidth); }
        ...
        // c * mask_all into the constant and remove both terms.
        {
            ...
            for (size_t i = 0; i < ir.terms.size(); ++i) {
                ...
                for (size_t j = i + 1; j < ir.terms.size(); ++j) {
                    ...
                    if (ir.terms[i].coeff != ir.terms[j].coeff) { continue; }
                    ...
                    for (size_t k = 0; k < ki.truth_table.size(); ++k) {
                        if (ki.truth_table[k] != ((~kj.truth_table[k]) & kMask)) {
                            complementary = false;
                            break;
                        }
                    }
                    ...
                    ir.constant = (ir.constant + (ir.terms[i].coeff * kMask)) & kMask;
                    removed[i]  = true;
                    removed[j]  = true;
                    break;
                }
            ...
            for (size_t i = 0; i < ir.terms.size(); ++i) {
                if (!removed[i]) { kept.push_back(ir.terms[i]); }
            }
