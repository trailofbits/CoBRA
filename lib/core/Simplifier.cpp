...
namespace cobra {
    ...
    namespace {
        ...
        Result< SimplifyOutcome > RunSupportedPipeline(
            ...
            if (real_var_count > opts.max_vars) {
            ...
            if (opts.evaluator) {
                if (elim.real_vars.size() == vars.size()) {
                    ...
                    ctx.eval     = [eval = opts.evaluator, idx_map = std::move(idx_map),
                                orig_sz = vars.size(
                                )](const std::vector< uint64_t > &reduced_vals) -> uint64_t {
        ...
        Evaluator MapEvaluator(
            ...
            if (real_vars.size() == vars.size()) {
                return eval;
            }
            if (real_vars.empty()) {
                return [eval,
                        orig_count = static_cast< uint32_t >(vars.size()
                        )](const std::vector< uint64_t > &) -> uint64_t {
                    return eval(std::vector< uint64_t >(orig_count, 0));
                };
            }
        ...
        std::unique_ptr< Expr >
        LowerNotOverArith(std::unique_ptr< Expr > e, uint32_t bitwidth) {
        ...
        bool IsVarProduct(const Expr &e) {
            ...
            const bool lhs_const = (e.children[0]->kind == Expr::Kind::kConstant);
            const bool rhs_const = (e.children[1]->kind == Expr::Kind::kConstant);
        ...
        void SplitAddTree(
            ...
            if (e.kind == Expr::Kind::kAdd && e.children.size() == 2) {
                ...
                } else if (rhs.kind == Expr::Kind::kNeg && rhs.children.size() == 1
        ...
        std::optional< SimplifyOutcome > TrySplitProductResidual(
            ...
            if (products.empty() || residual.empty()) {
                return std::nullopt;
            }
            ...
            const auto nv = static_cast< uint32_t >(vars.size());
            auto res_sig  = EvaluateBooleanSignature(*residual_expr, nv, opts.bitwidth);
            ...
            if (opts.evaluator) {
                auto check = FullWidthCheckEval(opts.evaluator, nv, *combined, opts.bitwidth);
                if (!check.passed) {
                    return std::nullopt;
                }
            }
            ...
            outcome.sig_vector = EvaluateBooleanSignature(*outcome.expr, nv, opts.bitwidth);
    ...
    Result< SimplifyOutcome > Simplify(
        ...
        if (input_expr == nullptr) {
            return RunSupportedPipeline(sig, vars, opts);
        }
        ...
        switch (cls.route) {
            ...
            case Route::kMixedRewrite: {
                ...
                {
                    const auto rv_count = static_cast< uint32_t >(
                    ...
                    auto check  = FullWidthCheckEval(
                        mapped, rv_count, *result.value().expr, opts.bitwidth
                    );
                    if (check.passed) {
                        result.value().diag.classification = cls;
                        return result;
                ...
                if (op_result.changed) {
                        ...
                        && reentry.value().kind == SimplifyOutcome::Kind::kSimplified)
                    {
                        ...
                        auto check = FullWidthCheckEval(
                            mapped, rv_count, *reentry.value().expr, opts.bitwidth
                        );
                        if (check.passed) {
                ...
                // Evaluator-based; does not depend on AST form.
                {
                    ...
                    const auto rv_count = static_cast< uint32_t >(elim.real_vars.size());
                ...
                if (!rewritten.structure_changed) {
                    return Ok(MakeUnchanged(working_expr, cls, rewritten, "No rewrite applied")
                    );
                }
