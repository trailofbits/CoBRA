...
namespace cobra {
    ...
    // Remainder prep passes: prepare RemainderStatePayload
    // for downstream solver passes.
    Result< PassResult > RunPrepareDirectRemainder(const WorkItem &, OrchestratorContext &);
    Result< PassResult > RunPrepareRemainderFromCore(const WorkItem &, OrchestratorContext &);

    // Decomposition residual solver passes: each consumes a
    // RemainderStatePayload, calls a solver, recombines with the prefix,
