...
namespace cobra {
    ...
    std::optional< SubResult > SimplifyFromSignature(
        ...
        const auto num_vars = static_cast< uint32_t >(ctx.vars.size());
        std::optional< SubResult > best;

        // Step 1: Fast-match canonical patterns on signature
        auto pm = MatchPattern(sig, num_vars, opts.bitwidth);
        if (pm) {
            bool pm_accepted = true;
            if (ctx.eval.has_value()) {
                auto check = FullWidthCheckEval(*ctx.eval, num_vars, **pm, opts.bitwidth);
                if (!check.passed) {
                    pm_accepted = false;
                }
            }
            ...
            if (pm_accepted) {
                ...
                if (opts.spot_check) {
                    auto check = SignatureCheck(sig, **pm, num_vars, opts.bitwidth);
                    if (!check.passed) {
                        pm_verified = false;
                        pm_accepted = false;
                    }
        ...
        if (IsBooleanValued(sig) && num_vars <= opts.max_vars) {
            auto anf      = ComputeAnf(sig, num_vars);
            auto anf_expr = BuildAnfExpr(anf, num_vars);

            bool anf_accepted = true;
            if (opts.spot_check) {
                auto check = SignatureCheck(sig, *anf_expr, num_vars, opts.bitwidth);
                if (!check.passed) {
                    anf_accepted = false;
                }
            }
            if (anf_accepted && ctx.eval.has_value()) {
                auto check = FullWidthCheckEval(*ctx.eval, num_vars, *anf_expr, opts.bitwidth);
                if (!check.passed) {
                    anf_accepted = false;
                }
            }
        ...
        auto coeffs = InterpolateCoefficients(sig, num_vars, opts.bitwidth);

        // Step 4: Build expression from CoB coefficients
        auto expr = BuildCobExpr(coeffs, num_vars, opts.bitwidth);
        ...
        if (ctx.eval.has_value()) {
            auto singleton_result = RecoverSingletonPowers(*ctx.eval, num_vars, opts.bitwidth);
            ...
            auto split =
                SplitCoefficients(coeffs, *ctx.eval, num_vars, opts.bitwidth, singleton_at_2);
            ...
            auto bit_expr = BuildCobExpr(residual, num_vars, opts.bitwidth);

            std::vector< std::unique_ptr< Expr > > pieces;
            const bool bit_is_zero = bit_expr && bit_expr->kind == Expr::Kind::kConstant
                && bit_expr->constant_val == 0;
            if (bit_expr && !bit_is_zero) {
                pieces.push_back(std::move(bit_expr));
            }
            if (poly_expr) {
                pieces.push_back(std::move(poly_expr));
            }
            if (singleton_expr) {
                pieces.push_back(std::move(singleton_expr));
            }
            ...
            auto check = FullWidthCheckEval(*ctx.eval, num_vars, *combined, opts.bitwidth);
            if (check.passed) {
        ...
        if (ctx.eval.has_value() && !poly_recovery_verified
            && HasFlag(opts.structural_flags, kSfHasMultivarHighPower) && num_vars <= 6)
        {
            std::vector< uint32_t > support(num_vars);
            for (uint32_t i = 0; i < num_vars; ++i) {
                support[i] = i;
            }
            auto multivar = RecoverMultivarPoly(*ctx.eval, support, num_vars, opts.bitwidth);
            if (multivar.has_value()) {
                auto poly_expr = BuildPolyExpr(*multivar);
                if (poly_expr.has_value()) {
                    auto check = FullWidthCheckEval(
                        *ctx.eval, num_vars, *poly_expr.value(), opts.bitwidth
                    );
                    if (check.passed) {
                        TryUpdateBest(
