...
namespace cobra {
        ...
        uint32_t CountActive(const std::vector< uint64_t > &g_sig, uint32_t n_g) {
            ...
            for (uint32_t v = 0; v < n_g; ++v) {
                ...
                for (size_t j = 0; j < g_sig.size(); ++j) {
                    const size_t flipped = j ^ (1U << v);
                    if (g_sig[j] != g_sig[flipped]) {
                        active = true;
                        break;
                    }
                }
    ...
    std::optional< SubResult > TryBitwiseDecomposition(
        ...
        const auto n      = static_cast< uint32_t >(ctx.vars.size());
        const size_t half = sig.size() / 2;
        ...
        for (uint32_t k = 0; k < n; ++k) {
            ...
            const bool all_cof0_zero =
                std::all_of(cof0.begin(), cof0.end(), [](uint64_t v) { return v == 0; });

            if (all_cof0_zero) {
                const uint32_t n_g = n - 1;
                const uint32_t ac  = CountActive(cof1, n_g);
                if (IsBooleanValued(cof1)) {
                    candidates.push_back(
                        { .var_k = k, .gate = GateKind::kAnd, .g_sig = cof1, .add_coeff = ac }
                    );
                }
                candidates.push_back(
                    { .var_k = k, .gate = GateKind::kMul, .g_sig = cof1, .add_coeff = ac }
                );
            }
            ...
            if (or_match) {
                const uint32_t n_g = n - 1;
                const uint32_t ac  = CountActive(cof0, n_g);
                candidates.push_back(
                    { .var_k = k, .gate = GateKind::kOr, .g_sig = cof0, .add_coeff = ac }
                );
            }
            ...
            if (xor_match) {
                ...
                const uint32_t ac  = CountActive(cof0, n_g);
                candidates.push_back(
                    { .var_k = k, .gate = GateKind::kXor, .g_sig = cof0, .add_coeff = ac }
                );
            }
            ...
            if (!all_cof0_zero && !cof0.empty()) {
                const uint64_t diff = cof1[0] - cof0[0];
                if (diff != 0) {
                    bool add_match = true;
                    for (size_t j = 1; j < cof0.size(); ++j) {
                        if ((cof1[j] - cof0[j]) != diff) {
                            add_match = false;
                            break;
                        }
                    }
                    if (add_match) {
                        const uint32_t n_g = n - 1;
                        const uint32_t ac  = CountActive(cof0, n_g);
                        candidates.push_back({ .var_k        = k,
                                               .gate         = GateKind::kAdd,
                                               .g_sig        = cof0,
                                               .add_coeff    = ac,
                                               .active_count = static_cast< uint32_t >(diff) });
                    }
                }
            }
        }
        ...
        for (const auto &cand : candidates) {
            ...
            if (active_var_indices.empty()) {
                ...
                {
                    auto check = FullWidthCheckEval(*ctx.eval, n, *composed, opts.bitwidth);
                    if (!check.passed) {
                        continue;
                    }
                }
            ...
            // Mul: g(rest) = f(rest, v_k=1)
            {
                ...
                g_ctx.eval = [eval = *ctx.eval, n_parent = n, k = cand.var_k, fixed_val,
                              compact_to_ctx](const std::vector< uint64_t > &sub_vals
                             ) -> uint64_t {
