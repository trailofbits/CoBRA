...
namespace cobra {
    ...
    namespace {
        ...
        constexpr Gate kAllGates[] = { Gate::kAnd, Gate::kOr, Gate::kXor, Gate::kAdd,
                                       Gate::kMul };
        ...
        ProbeVals
        Probe(const Expr &e, const std::vector< std::vector< uint64_t > > &pts, uint32_t bw) {
            ProbeVals v;
        ...
        std::optional< SubResult > Layer2(
            ...
            for (auto g_in : kAllGates) {
                for (size_t bi = 0; bi < pool.size(); ++bi) {
                    for (size_t ci = bi; ci < pool.size(); ++ci) {
                        ...
                        if (vmap.contains(v) != 0u) {
                            continue;
                        }
                        if (inner_idx.contains(v) != 0u) {
                            continue;
                        }
            ...
            for (auto g_out : kAllGates) {
                if (GateInvertible(g_out)) {
                    ...
                    for (const auto &ai : pool) {
                        if (!Compatible(ai.vals, target, g_out)) {
                            continue;
                        }
                        for (auto &ii : inner) {
    ...
    std::optional< SubResult > TryTemplateDecomposition(
        ...
        if (!ctx.eval || num_vars > kMaxVars) {
            return std::nullopt;
        }

        // Zero real variables: the function is a constant.
        if (num_vars == 0) {
            const uint64_t val = (*ctx.eval)({});
            auto e             = Expr::Constant(val);
        ...
        // Layer 1.
        auto r1 =
            Layer1(target, pool, vmap, mask, *ctx.eval, num_vars, opts.bitwidth, baseline_cost);
        if (r1.has_value()) {
            return r1;
        }
        ...
        // Layer 2.
        auto r2 =
            Layer2(target, pool, vmap, mask, *ctx.eval, num_vars, opts.bitwidth, baseline_cost);
        if (r2.has_value()) {
            return r2;
        }
        ...
        for (int wrap = 0; wrap < 2; ++wrap) {
            ...
            for (size_t i = 0; i < kNProbes; ++i) {
                if (wrap == 0) {
                    lifted[i] = (-target[i]) & mask; // Neg
                } else {
                    lifted[i] = (~target[i]) & mask; // Not
                }
            }
            ...
            // Layer 1 on lifted
            auto w1 =
                Layer1(lifted, pool, vmap, mask, *ctx.eval, num_vars, opts.bitwidth, nullptr);
            if (w1.has_value()) {
