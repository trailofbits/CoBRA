...
namespace cobra {
    ...
    SelfCheckResult SelfChecSemilinear(
        ...
        const CoeffMap orig_map = BuildCoeffMap(original_ir, kMask);
        CoeffMap re_map         = BuildCoeffMap(re, kMask);

        if (orig_map.size() != re_map.size()) {
            std::ostringstream oss;
            oss << "term count mismatch: original=" << orig_map.size()
                << " re=" << re_map.size();
            return { .passed = false, .mismatch_detail = oss.str() };
        }

        for (const auto &[key, coeff] : orig_map) {
