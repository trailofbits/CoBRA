    ...
    std::unique_ptr< Expr > ReconstructMaskedAtoms(
        ...
        if (!partitions.empty()) {
            for (size_t i = 0; i < entries.size(); ++i) {
                ...
                const uint64_t mask_i = ComputeActiveMask(entries[i].atom_id, partitions);

                for (size_t j = i + 1; j < entries.size(); ++j) {
                    if (consumed[j] || entries[j].coeff != 1) {
                        continue;
                    }
                    const uint64_t mask_j = ComputeActiveMask(entries[j].atom_id, partitions);
        ...
        for (auto &c : combined) {
            all_terms.push_back(std::move(c));
        }
        for (size_t i = 0; i < entries.size(); ++i) {
            if (!consumed[i]) {
                all_terms.push_back(std::move(entries[i].expr));
            }
        }
