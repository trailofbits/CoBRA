...
namespace cobra {
    ...
    namespace {
            ...
            switch (e.kind) {
                ...
                case Expr::Kind::kNot:
                    return EvalAtomAtBitImpl(
                        ...
                        ^ 1;
                case Expr::Kind::kShr: {
                    const uint32_t src = bit_pos + static_cast< uint32_t >(e.constant_val);
                    if (src >= bitwidth) {
                        return 0;
                    }
                    return EvalAtomAtBitImpl(
                        *e.children[0], support, assignment, src, bitwidth
                    );
                }
                case Expr::Kind::kAdd:
                case Expr::Kind::kMul:
        ...
        uint64_t EvalAtomAtBit(
            ...
            for (size_t i = 0; i < len; ++i) {
                const uint64_t val  = EvalAtomAtBitImpl(atom, support, i, bit_pos, bitwidth);
                packed             |= (val & 1) << i;
            }
            return packed;
        }
    ...
    std::vector< PartitionClass > ComputePartitions(const SemilinearIR &ir) {
        ...
        const size_t atom_count = ir.atom_table.size();
        ...
        std::vector< AtomMeta > meta(atom_count);
        ...
        std::vector< AtomSemanticId > profile(atom_count);
