...
namespace cobra {
        ...
        void RenderImpl(
            ...
            switch (expr.kind) {
                case Expr::Kind::kConstant: {
                    ...
                    if (expr.constant_val >= half && expr.constant_val <= mask) {
                        const uint64_t neg = (mask - expr.constant_val) + 1;
                        out << "-" << neg;
                    } else {
                ...
                case Expr::Kind::kNeg:
                    ...
                    break;
                case Expr::Kind::kShr: {
                    const int prec          = Precedence(Expr::Kind::kShr);
                    const bool needs_parens = prec > parent_prec && parent_prec > 0;
                    if (needs_parens) {
                        out << "(";
                    }
                    RenderImpl(out, *expr.children[0], var_names, bitwidth, prec);
                    out << " >> " << expr.constant_val;
                    if (needs_parens) {
                        out << ")";
                    }
                ...
                default: {
                    const int prec          = Precedence(expr.kind);
                    const bool needs_parens = prec > parent_prec && parent_prec > 0;
                    if (needs_parens) {
                        out << "(";
                    }
                    ...
                    RenderImpl(out, *expr.children[1], var_names, bitwidth, prec);
                    if (needs_parens) {
                        out << ")";
                    }
                    break;
    ...
    std::unique_ptr< Expr >
    Expr::Add(std::unique_ptr< Expr > lhs, std::unique_ptr< Expr > rhs) {
        return MakeBinary(Kind::kAdd, std::move(lhs), std::move(rhs));
    }

    std::unique_ptr< Expr >
    Expr::Mul(std::unique_ptr< Expr > lhs, std::unique_ptr< Expr > rhs) {
        return MakeBinary(Kind::kMul, std::move(lhs), std::move(rhs));
    }

    std::unique_ptr< Expr >
    Expr::BitwiseAnd(std::unique_ptr< Expr > lhs, std::unique_ptr< Expr > rhs) {
        return MakeBinary(Kind::kAnd, std::move(lhs), std::move(rhs));
    }

    std::unique_ptr< Expr >
    Expr::BitwiseOr(std::unique_ptr< Expr > lhs, std::unique_ptr< Expr > rhs) {
        return MakeBinary(Kind::kOr, std::move(lhs), std::move(rhs));
    }

    std::unique_ptr< Expr >
    Expr::BitwiseXor(std::unique_ptr< Expr > lhs, std::unique_ptr< Expr > rhs) {
    ...
    std::unique_ptr< Expr > CloneExpr(const Expr &expr) {
        ...
        for (const auto &child : expr.children) {
            dst->children.push_back(CloneExpr(*child));
        }
        return dst;
    }
