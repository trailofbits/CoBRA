...
namespace cobra {
    ...
    namespace {
        ...
        bool MonomialLess(uint32_t a, uint32_t b) {
            const int da = std::popcount(a);
            const int db = std::popcount(b);
            if (da != db) {
                return da < db;
            }
            return a < b;
        ...
        uint32_t MonomialExprCost(uint32_t mask) {
            ...
            return static_cast< uint32_t >((2 * pc) - 1);
        }
        ...
        uint32_t RawAnfCost(const std::vector< uint32_t > &monomials, uint8_t constant_bit) {
            const uint32_t n = static_cast< uint32_t >(monomials.size()) + constant_bit;
            if (n == 0) {
                return 1;
            }
            uint32_t leaf_cost = constant_bit;
            for (const uint32_t m : monomials) {
                leaf_cost += MonomialExprCost(m);
            }
            return leaf_cost + (n > 1 ? n - 1 : 0);
        }
        ...
        uint32_t OrChainCost(uint32_t var_mask) {
            const int k = std::popcount(var_mask);
            if (k <= 0) {
                return 1;
            }
            if (k == 1) {
        ...
        std::unique_ptr< Expr >
        XorTree(std::vector< std::unique_ptr< Expr > > &terms, size_t lo, size_t hi) {
            if (hi - lo == 1) {
                return std::move(terms[lo]);
            }
        ...
        std::optional< uint32_t > DetectOrFamily(const std::vector< uint32_t > &monomials) {
            ...
            const int nv          = std::popcount(var_union);
            const size_t expected = (1ULL << nv) - 1;
            if (monomials.size() != expected) {
                return std::nullopt;
            }
            ...
            const size_t total = 1ULL << bits.size();
        ...
        std::optional< FactorCandidate > FindBestFactor(const AnfForm &form) {
            ...
            for (size_t i = 0; i < form.monomials.size(); ++i) {
                for (size_t j = i + 1; j < form.monomials.size(); ++j) {
                    const uint32_t common = form.monomials[i] & form.monomials[j];
                    if (common != 0 && std::popcount(common) >= 2) {
                        candidates.push_back(common);
                    }
                }
            }
            ...
            for (const uint32_t cand : candidates) {
                ...
                const uint32_t raw_cost = RawAnfCost(covered_monos, 0);
                ...
                if (factored_cost < raw_cost) {
                    const uint32_t saving = raw_cost - factored_cost;
        ...
        FindPartialOr(const std::vector< uint32_t > &monomials) {
            ...
            const int nv = static_cast< int >(var_bits.size());
            if (nv < 2) {
                return std::nullopt;
            }
            ...
            const uint32_t limit = 1U << nv;
            for (uint32_t s = 3; s < limit; ++s) {
                const int subset_size = std::popcount(s);
                if (subset_size < 2) {
                    continue;
                }
                ...
                const uint32_t family_size = (1ULL << subset_size) - 1;
                // Skip if this covers all monomials (Rule 1 handles)
                if (family_size >= monomials.size()) {
                    continue;
                }
                ...
                std::vector< uint32_t > family_masks;
                family_masks.reserve(family_size);
                ...
                const uint32_t oc       = OrChainCost(var_mask);
                const uint32_t raw_cost = RawAnfCost(family_masks, 0);

                if (oc < raw_cost) {
                    const uint32_t saving = raw_cost - oc;
                    if (saving > best_saving) {
                        best_saving = saving;
        ...
        std::optional< AbsorptionCandidate > FindAbsorption(const AnfForm &form) {
            ...
            const uint32_t a = form.monomials[0];
            const uint32_t b = form.monomials[1];

            const uint32_t raw_cost = RawAnfCost(form.monomials, form.constant_bit);

            // Check if a is subset of b (a is M, b is M&N)
            if ((a & b) == a && a != b) {
                const uint32_t n               = b & ~a;
                // M & ~N cost: 1 (AND) + monomial(M) + 1 (NOT) + monomial(N)
                const uint32_t absorption_cost = 2 + MonomialExprCost(a) + MonomialExprCost(n);

                if (absorption_cost < raw_cost) {
                    return AbsorptionCandidate{ .m_idx = 0, .mn_idx = 1, .n_mask = n };
                }
            }
            ...
            if ((b & a) == b && b != a) {
                ...
                const uint32_t absorption_cost = 2 + MonomialExprCost(b) + MonomialExprCost(n);

                if (absorption_cost < raw_cost) {
                    return AbsorptionCandidate{ .m_idx = 1, .mn_idx = 0, .n_mask = n };
                }
            }
    ...
    AnfForm AnfForm::FromAnfCoeffs(const PackedAnf &anf, uint32_t num_vars) {
        ...
        form.constant_bit = (!anf.Empty() && (anf[0] != 0u)) ? 1 : 0;
        for (size_t w = 0; w < anf.WordCount(); ++w) {
            ...
            while (bits != 0u) {
                ...
                form.monomials.push_back(static_cast< uint32_t >((w * 64) + bit));
    ...
    std::unique_ptr< Expr > EmitRawAnf(const AnfForm &form) {
        ...
        if (form.constant_bit != 0u) {
            terms.push_back(Expr::Constant(1));
        }
        for (const uint32_t mask : form.monomials) {
            terms.push_back(BuildMonomial(mask));
        }
        if (terms.empty()) {
    ...
    uint32_t ExprCost(const Expr &expr) {
        switch (expr.kind) {
            ...
            case Expr::Kind::kOr: {
                ...
                if (expr.kind == Expr::Kind::kOr) {
                    const uint32_t depth = CountOrChainDepth(expr);
                    if (depth >= 2) {
                        base -= 1;
                    }
                }
    ...
    std::unique_ptr< Expr > CleanupAnf(const AnfForm &form) {
        if (form.monomials.empty()) {
            return Expr::Constant(form.constant_bit);
        }
        ...
        const uint32_t raw_cost = ExprCost(*raw);
        std::unique_ptr< Expr > best;
        uint32_t best_cost = raw_cost;
        ...
        if (partial.has_value()) {
            ...
            for (const uint32_t m : form.monomials) {
                if (!family_set.contains(m)) {
                    remainder.monomials.push_back(m);
                }
            }
            ...
            const uint32_t cost = ExprCost(*result);
            if (cost < best_cost) {
                best_cost = cost;
        ...
        if (factor.has_value()) {
            ...
            for (size_t i = 0; i < form.monomials.size(); ++i) {
                if (!used[i]) {
                    remainder.monomials.push_back(form.monomials[i]);
                }
            }
