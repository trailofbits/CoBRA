...
namespace cobra {
    ...
    std::optional< NormalizedPoly > RecoverMultivarPoly(
        ...
        // Compute 3^kK (table size)
        uint32_t table_size = 1;
        for (uint32_t i = 0; i < kK; ++i) { table_size *= 3; }

        // Evaluate on {0,1,2}^kK grid.
        ...
        NormalizedPoly result; // NOLINT(misc-const-correctness)
        result.num_vars = nv;
        ...
        for (uint32_t idx = 0; idx < table_size; ++idx) {
            ...
            uint32_t tmp = idx;
            for (uint32_t i = 0; i < kK; ++i) {
                const auto kE         = static_cast< uint8_t >(tmp % 3);
            ...
            uint64_t h                = kAlpha >> q;
            const uint32_t kBoundBits = bitwidth - q;
            if (kBoundBits < 64) { h &= (1ULL << kBoundBits) - 1; }

            if (h == 0) { continue; }

            auto key           = ExponentTuple::FromExponents(exps.data(), nv);
            result.coeffs[key] = h;
        }
