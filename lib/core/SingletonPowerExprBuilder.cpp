...
namespace cobra {
    ...
    std::vector< uint64_t >
    factorial_to_monomial(const std::vector< UnivariateTerm > &terms, uint32_t bitwidth) {
        if (terms.empty()) {
            return {};
        }
        ...
        for (uint16_t k = 1; k <= d_max; ++k) {
            ...
            const auto km1 = static_cast< uint64_t >(k - 1);
            for (uint16_t j = 0; j <= k; ++j) {
                const uint64_t from_left = (j > 0) ? s_prev[j - 1] : 0;
                const uint64_t from_diag = s_prev[j];
                // Modular: (from_left - km1 * from_diag) mod 2^w.
                // Unsigned subtraction wraps correctly for all bitwidths.
                s_curr[j]                = (from_left - ((km1 * from_diag) & mask)) & mask;
            }

            // Accumulate h[k] * s(k, j) into mono[j].
            if (h[k] != 0) {
                for (uint16_t j = 1; j <= k; ++j) {
                    mono[j] = (mono[j] + (h[k] * s_curr[j])) & mask;
                }
            }
    ...
    std::unique_ptr< Expr > BuildSingletonPowerExpr(const SingletonPowerResult &powers) {
        const uint32_t w      = powers.bitwidth;
        const size_t num_vars = powers.per_var.size();
        ...
        for (size_t i = 0; i < num_vars; ++i) {
            ...
            auto mono = factorial_to_monomial(uni.terms, w);
            ...
            for (size_t j = 1; j < mono.size(); ++j) {
                ...
                if (mono[j] != 0) {
                    ...
                    if (j + 1 < mono.size()) {
                        degree_exprs.push_back(ApplyCoefficient(CloneExpr(*power), mono[j], w));
                    } else {
                        degree_exprs.push_back(ApplyCoefficient(std::move(power), mono[j], w));
                    }
                }
            }

            if (degree_exprs.empty()) {
                continue;
            }
