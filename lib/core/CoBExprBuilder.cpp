...
namespace cobra {
    ...
    namespace {
            ...
            for (size_t si = 0; si < sorted.size(); ++si) {
                ...
                for (size_t sj = si + 1; sj < sorted.size(); ++sj) {
                    ...
                    if (c_union == neg_c) {
                        ...
                        if (union_idx < terms.size()) {
                            terms[union_idx].consumed = true;
                        }

                        terms.push_back({ .mask     = m_union,
                                          .coeff    = c,
        ...
        bool
        TryNotRecognition(std::vector< Term > &terms, uint64_t const_coeff, uint32_t bitwidth) {
            ...
            for (size_t i = 0; i < terms.size(); ++i) {
                if (terms[i].consumed || terms[i].mask == 0) {
                    continue;
                }
                if (terms[i].coeff != neg1) {
                    return false;
                }
                if (match < terms.size()) {
    ...
    std::unique_ptr< Expr > BuildCobExpr(
        ...
        for (size_t i = 1; i < coeffs.size(); ++i) {
            ...
            const size_t idx = terms.size();
            terms.push_back({ .mask     = static_cast< uint64_t >(i),
                              .coeff    = coeffs[i],
                              .expr     = nullptr,
                              .consumed = false });
            mask_index[static_cast< uint64_t >(i)] = idx;
        }
        ...
        const bool used_not = TryNotRecognition(terms, const_coeff, bitwidth);
        if (used_not) {
        ...
        if (const_coeff != 0) {
            result = Expr::Constant(const_coeff);
        }
