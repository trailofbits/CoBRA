...
namespace cobra {
    ...
    CostInfo ComputeCost(const Expr &expr) {
        switch (expr.kind) {
            ...
            case Expr::Kind::kMul: {
                auto lhs              = ComputeCost(*expr.children[0]);
                auto rhs              = ComputeCost(*expr.children[1]);
                const bool nonlinear  = lhs.has_var_dep && rhs.has_var_dep;
                const uint32_t weight = nonlinear ? 3 : 1;
                uint32_t nl_count = lhs.cost.nonlinear_mul_count + rhs.cost.nonlinear_mul_count
                    + (nonlinear ? 1 : 0);
                return {
                    .cost        = { .weighted_size =
                                         lhs.cost.weighted_size + rhs.cost.weighted_size + weight,
    ...
    bool IsBetter(const ExprCost &a, const ExprCost &b) {
        return std::tie(a.weighted_size, a.nonlinear_mul_count, a.max_depth)
            < std::tie(b.weighted_size, b.nonlinear_mul_count, b.max_depth);
    }
