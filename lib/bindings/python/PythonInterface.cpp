...
NB_MODULE(cobra_mba, m) {
  ...
  nb::class_<cobra::py::PyExpr>(m, "ExprNode")
      ...
      .def_ro("constant_val", &cobra::py::PyExpr::constant_val,
              "If a node is a constant, this will hold its value, undefined "
              "otherwise")
      .def_ro("var_index", &cobra::py::PyExpr::var_index,
              "The variables no. true name can be recoved by using it as an "
              "index into the var "
              "list, undefined if the node is not a variable")
      .def_ro("children", &cobra::py::PyExpr::children,
              "List of child nodes of the expression node, such as the addends "
              "of an addition "
              "node. Will be empty for constant and variable nodes");

  nb::class_<cobra::py::PyExprTree>(m, "Expr")
      .def(nb::init<const cobra::py::PyExpr &, std::vector<std::string>,
                    uint32_t>(),
           "root"_a, "vars"_a, "bitwidth"_a = 64,
           "Create an expression tree from a root node, its variable names and "
           "bitwidth")

      .def(nb::init<const std::string &, uint32_t, uint32_t>(), "expr"_a,
           "max_vars"_a = 16, "bitwidth"_a = 64,
           "Parse an expression from a string. Warning increasing max_vars "
           "past 16 may cause OOM errors")
      ...
      .def_static( // duplicate of the init method but with a nicer name
          ...
          "Parse an expression from a string. Warning increasing max_vars past "
          "16 may cause OOM errors")

      .def("__str__", &cobra::py::PyExprTree::ToString)
      .def("simplify", &cobra::py::PyExprTree::Simplify, "validate"_a = false,
           "Simplify the expression in place. When validate=true, run "
           "full-width checks")
      ...
      .def_rw("bitwidth", &cobra::py::PyExprTree::bitwidth,
              "Bitwidth of the expression");
}
